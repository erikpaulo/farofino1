define(['./module'
	...
	app.controller('AccountSTKController', ['$scope', '$interval', '$filter', '$mdDialog', 'AccountResource', 'AccountEntryResource', 'IndexResource', 'StockSaleProfitResource', 'Utils', 'Constants',
        function($scope, $interval, $filter, $mdDialog, Account, AccountEntry, Index, StockSaleProfit, Utils, Constants) {
            ...
            $scope.appContext.contextMenu.addAction(
                {icon: 'strikethrough_s', tooltip: 'Pagar IR', onClick: function() {
                    $scope.payIncomeTax();
                }}
            );
            $scope.appContext.contextMenu.addAction(
                {icon: 'monetization_on', tooltip: 'Registrar Dividendo', onClick: function() {
                    $scope.registerDividend();
                }}
            );
            ...
            $scope.registerDividend = function(){
                $mdDialog.show({
                    controller: DialogDividendController,
                    templateUrl: 'modules/account/views/new-dividend-STK-template.html',
                    parent: angular.element(document.body),
                    clickOutsideToClose:true
                }).then(function(newDividend){
                    newDividend.type = 'STK';
                    newDividend.accountId = $scope.account.id;

                    var newDividend = new Dividend(newDividend);
                    newDividend.$save(function(e){
                        getAccountDetail()
                    }, function(err){
...
//                    stockSaleProfit.accountId = $scope.account.id;
            ...
            function DialogDividendController($scope, $mdDialog, Utils) {
                ...
                $scope.submit = function() {
                    $scope.dividend.value = Utils.currencyToNumber($scope.dividend.value);
                    $mdDialog.hide($scope.dividend);
                };
            }
